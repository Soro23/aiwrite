...
export default function SqlEditorPage() {
  ...
  const runQuery = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Refresh the tables sidebar if any DDL statement was executed
        const hasDdl = (data.data as SqlResult[]).some((r) => r.type === "affected");
        if (hasDdl) setRefreshTrigger((n) => n + 1);
      } else {
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      <DatabaseTablesSidebar databaseId={params.id} refreshTrigger={refreshTrigger} />
