...
export default function PoliciesPage() {
  ...
  const [sql, setSql] = useState(POLICY_TEMPLATE);
  const [creating, setCreating] = useState(false);
  const [createError, setCreateError] = useState<string | null>(null);
  const [dropping, setDropping] = useState<PolicyInfo | null>(null);
  const [dropLoading, setDropLoading] = useState(false);

  const load = () => {
    setLoading(true);
    fetch(`/api/databases/${params.id}/policies`)
      .then((r) => r.json())
  ...
  const handleCreate = async () => {
    setCreating(true);
    setCreateError(null);
    const res = await fetch(`/api/databases/${params.id}/policies`, {
      method: "POST",
    ...
    setCreating(false);
    if (d.success) { setShowCreate(false); setSql(POLICY_TEMPLATE); load(); }
  ...
  const handleDrop = async () => {
    if (!dropping) return;
    setDropLoading(true);
    await fetch(`/api/databases/${params.id}/policies/${encodeURIComponent(dropping.name)}`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
  ...
  const grouped = policies.reduce<Record<string, PolicyInfo[]>>((acc, p) => {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="px-6 py-3 border-b border-[#2e2e2e] flex items-center justify-between">
        <span className="text-xs font-medium text-[#888]">Row Level Security Policies</span>
      ...
      {policies.length === 0 && !loading && (
        <div className="mx-6 mt-4 p-3 bg-[#1a1200] border border-[#3a2c00] rounded text-xs text-[#aa8800]">
          Note: RLS must be enabled on the table first (ALTER TABLE ... ENABLE ROW LEVEL SECURITY).
        </div>
      )}
      ...
      {loading ? (
        ...
        <EmptyState
          icon={ShieldCheck}
          title="No policies"
          description="Create row-level security policies to control data access per row."
          action={{ label: "New policy", onClick: () => setShowCreate(true) }}
        />
      ) : (
        <div className="flex-1 overflow-y-auto p-6">
          {Object.entries(grouped).map(([table, pols]) => (
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-[#2e2e2e] bg-[#141414]">
                      <th className="text-left px-4 py-2 text-[#555] font-normal">Policy</th>
                      <th className="text-left px-4 py-2 text-[#555] font-normal">Command</th>
                      <th className="text-left px-4 py-2 text-[#555] font-normal">Roles</th>
                      <th className="text-left px-4 py-2 text-[#555] font-normal">USING</th>
                      <th className="w-10" />
                    </tr>
                  </thead>
                  <tbody>
                    {pols.map((p) => (
                      <tr key={p.name} className="border-b border-[#1e1e1e] last:border-0 hover:bg-[#141414]">
                        <td className="px-4 py-2.5 text-[#ccc]">{p.name}</td>
                        <td className="px-4 py-2.5 text-[#888]">{p.command}</td>
                        <td className="px-4 py-2.5 text-[#888]">{p.roles.join(", ") || "—"}</td>
                        <td className="px-4 py-2.5 text-[#888] font-mono text-[10px]">{p.usingExpr ?? "—"}</td>
                        <td className="px-2 py-2.5">
                          <button onClick={() => setDropping(p)} className="p-1 text-[#444] hover:text-red-400">
                            <Trash2 size={12} />
                          </button>
                        </td>
                      </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-[#1a1a1a] border border-[#2e2e2e] rounded-lg w-full max-w-2xl mx-4 shadow-xl flex flex-col" style={{ maxHeight: "80vh" }}>
            <div className="px-5 py-4 border-b border-[#2e2e2e] flex items-center justify-between">
              <h2 className="text-sm font-semibold text-[#ededed]">Create policy</h2>
      ...
      <ConfirmDropModal
        ...
        description={`This will remove the policy from table "${dropping?.tableName}".`}
